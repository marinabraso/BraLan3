...
Scatter_plot <- function(vec1, vec2, xlim, ylim, names, col, lab1, lab2){
	plot(c(1:10), c(1:10), axes=F, xlab="", ylab="", ylim=c(ylim[1],ylim[2]+(ylim[2]-ylim[1])*.05), xlim=c(xlim[1],xlim[2]+(xlim[2]-xlim[1])*.05), col=NA)
	...
	axis(1, at = seq(xlim[1],xlim[2],(xlim[2]-xlim[1])/5),  lwd.ticks=1, las=1, cex.axis=.7)
	axis(2, at = seq(ylim[1],ylim[2],(ylim[2]-ylim[1])/5), lwd.ticks=1, las=1, cex.axis=.7)
	box()
}
