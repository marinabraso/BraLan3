...
## getting the vector of gene_biotypes
biotypes <- sapply(split_annotation_list, function(x){ get_annot_value(x, 'gene_biotype') })

## getting the table with mappings between biotypes and gene IDs (for export)
gene_biotypes <- unique(cbind(gene_ids, biotypes), MARGIN=1)
...
write.table(gene_biotypes, file=paste(output_gtf_path, ".gene2biotype", sep=""), sep="\t", row.names=FALSE, col.names=FALSE, quote=FALSE)
