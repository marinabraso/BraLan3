...
# Calc parameters per ortogroup per sepcies
for(s in c(1:length(Species))){
...
GeneCN$TypeVert[which(GeneCN$OG %in% OnhOG.S)] <- "O"
...
GenePairs$Type2[which(GenePairs$OG %in% OnhOG.S)] <- "O"
...
layout(matrix(c(1,2,3,4,5,6),nrow=2,ncol=3,byrow=T), widths=c(1), heights=c(1), TRUE)
ScatterPlot(GO$BlanD.AExp, GO$BlanSC.AExp, "black", "Mean adult expression\nof duplicate genes", "Mean adult expression\nof single-copy genes", c(0,100), c(0,100))
ScatterPlot(GO$BlanD.EExp, GO$BlanSC.EExp, "black", "Mean embrionic expression\nof duplicate genes", "Mean embrionic expression\nof single-copy genes", c(0,100), c(0,100))
